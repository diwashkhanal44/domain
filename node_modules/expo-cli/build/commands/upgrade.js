...
async function makeBreakingChangesToConfigAsync(projectRoot, targetSdkVersionString) {
  ...
  let step = logNewSection('Updating your app.json to account for breaking changes (if applicable)...');
  let {
    rootConfig
    ...
    switch (targetSdkVersionString) {
      case '37.0.0':
        if ((rootConfig === null || rootConfig === void 0 ? void 0 : (_rootConfig$expo = rootConfig.expo) === null || _rootConfig$expo === void 0 ? void 0 : (_rootConfig$expo$andr = _rootConfig$expo.androidNavigationBar) === null || _rootConfig$expo$andr === void 0 ? void 0 : _rootConfig$expo$andr.visible) !== undefined) {
          ...
          step.stopAndPersist({
            symbol: '⚠️ ',
            text: _chalk().default.red(`Please manually update "androidNavigationBar.visible" according to ${(0, _terminalLink().default)('this documentation', 'https://docs.expo.io/workflow/configuration/#androidnavigationbar')}`)
          });
        } else {
          step.succeed('No additional changes necessary to app.json config.');
        }
...
async function maybeUpgradeSimulatorAsync() {
  ...
  if (_xdl().Simulator.isPlatformSupported()) {
    if (await shouldUpgradeSimulatorAsync()) {
      let result = await _xdl().Simulator.upgradeExpoAsync();
...
async function upgradeAsync({
  ...
  let sdkVersions = await _xdl().Versions.releasedSdkVersionsAsync();
  let latestSdkVersion = await _xdl().Versions.newestReleasedSdkVersionAsync();
  ...
  if (targetSdkVersionString === latestSdkVersionString && currentSdkVersionString !== targetSdkVersionString && !_commander().default.nonInteractive) {
    ...
    if (!answer.updateToLatestSdkVersion) {
      let sdkVersionStringOptions = Object.keys(sdkVersions).filter(v => _semver().default.lte('33.0.0', v) && !_xdl().Versions.gteSdkVersion(exp, v));
      let {
  ...
  if (!_xdl().Versions.gteSdkVersion(currentExp, targetSdkVersionString) && currentExp.sdkVersion !== 'UNVERSIONED') {
    ...
    removingSdkVersionStep.warn('Please manually delete the sdkVersion field in your project app.config file, it is deprecated.');
  }

  await makeBreakingChangesToConfigAsync(projectRoot, targetSdkVersionString);
  let updatingPackagesStep = logNewSection('Updating packages to compatible versions (where known).');
  ...
  if (skippedSdkVersionKeys.length) {
    ...
    let releaseNotesUrls = Object.values(skippedSdkVersions).map(data => data.releaseNoteUrl).filter(releaseNoteUrl => releaseNoteUrl).reverse();
...
async function maybeCleanNpmStateAsync(packageManager) {
  ...
  if (packageManager instanceof PackageManager().NpmPackageManager) {
    let cleaningNpmStateStep = logNewSection('Removing package-lock.json and deleting node_modules.');
    ...
    if (shouldInstallNodeModules) {
      let reinstallingNodeModulesStep = logNewSection('Installing node_modules and rebuilding package-lock.json.');
...
function _default(program) {
  program.command('upgrade [targetSdkVersion]').alias('update').option('--npm', 'Use npm to install dependencies. (default when package-lock.json exists)').option('--yarn', 'Use Yarn to install dependencies. (default when yarn.lock exists)').description('Upgrades your project dependencies and app.json and to the given SDK version').asyncAction(async (requestedSdkVersion, options) => {
    const {
      projectRoot,
      workflow
