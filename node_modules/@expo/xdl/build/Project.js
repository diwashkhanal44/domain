...
function _requireFromProject(modulePath, projectRoot, exp) {
  try {
    let fullPath = (0, _config().resolveModule)(modulePath, projectRoot, exp); // Clear the require cache for this module so get a fresh version of it
...
async function getSlugAsync(projectRoot) {
  const {
    exp
  ...
  throw new (_XDLError().default)('INVALID_MANIFEST', `Your project config in ${projectRoot} must contain a "slug" field. Please supply this in your app.config.js or app.json`);
}
...
async function getLatestReleaseAsync(projectRoot, options) {
  ...
  if (process.env.EXPO_LEGACY_API === 'true') {
    ...
    result = await _Api().default.callMethodAsync('publishInfo', [], 'post', null, {
      formData
    });
  } else {
...
async function exportForAppHosting(projectRoot, publicUrl, assetUrl, outputDir, options = {}) {
  ...
  let androidSourceMap = null; // build source maps

  if (options.dumpSourcemap) {
    ({
      iosSourceMap,
      androidSourceMap
    } = await _buildSourceMapsAsync(projectRoot)); // write the sourcemap files
...
async function publishAsync(projectRoot, options = {}) {
  ...
  const user = await _User().default.ensureLoggedInAsync();
  const target = (_options$target = options.target) !== null && _options$target !== void 0 ? _options$target : (0, _config().getDefaultTarget)(projectRoot);
  await _validatePackagerReadyAsync(projectRoot);
...
async function _uploadArtifactsAsync({
  ...
  if (process.env.EXPO_LEGACY_API === 'true') {
    response = await _Api().default.callMethodAsync('publish', null, 'put', null, {
      formData
    });
  } else {
    const user = await _User().default.ensureLoggedInAsync();
...
async function _buildSourceMapsAsync(projectRoot) {
...
async function uploadAssetsAsync(projectRoot, assets) {
  ...
  if (process.env.EXPO_LEGACY_API === 'true') {
    result = await _Api().default.callMethodAsync('assetsMetadata', [], 'post', {
    ...
    result = await api.postAsync('assets/metadata', {
      keys: Object.keys(paths)
    });
  }
...
async function startReactNativeServerAsync(projectRoot, options = {}, verbose = true) {
  ...
  const customLogReporterPath = require.resolve(_path().default.join(__dirname, 'reporter')); // TODO: Bacon: Support .mjs (short-lived JS modules extension that some packages use)
  ...
  if (userPackagerOpts) {
    ...
    packagerOpts = { ...packagerOpts,
      ...
      sourceExts: (0, _uniq().default)([...packagerOpts.sourceExts, ...((_userPackagerOpts$sou = userPackagerOpts.sourceExts) !== null && _userPackagerOpts$sou !== void 0 ? _userPackagerOpts$sou : [])])
    };
  ...
  let defaultCliPath = (0, _config().resolveModule)('react-native/local-cli/cli.js', projectRoot, exp);
...
function getManifestHandler(projectRoot) {
  return async (req, res) => {
    try {
      ...
      // down the request
      Doctor().validateWithNetworkAsync(projectRoot); // Get packager opts and then copy into bundleUrlPackagerOpts
      ...
      manifest.bundleUrl = (await UrlUtils().constructBundleUrlAsync(projectRoot, bundleUrlPackagerOpts, hostname)) + path;
      ...
      if (req.headers['exponent-accept-signature']) {
        if (_cachedSignedManifest.manifestString === manifestString) {
          ...
          if (!currentSession || _Config().default.offline) {
            ...
            if (process.env.EXPO_LEGACY_API === 'true') {
              signedManifest = await _Api().default.callMethodAsync('signManifest', [publishInfo.args], 'post', manifest);
            } else {
              const user = await _User().default.ensureLoggedInAsync();
...
async function startExpoServerAsync(projectRoot) {
  ...
  if ((_xdl().ConnectionStatus.isOffline() ? await Doctor().validateWithoutNetworkAsync(projectRoot) : await Doctor().validateWithNetworkAsync(projectRoot)) === Doctor().FATAL) {
  ...
  let expRc = await (0, _config().readExpRcAsync)(projectRoot);
